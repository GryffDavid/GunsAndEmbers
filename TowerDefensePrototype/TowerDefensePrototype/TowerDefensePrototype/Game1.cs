...
namespace TowerDefensePrototype
{
    ...
    /// <summary>
    /// Whether something being drawn has no lighting mapping, a normal map or an emissive map
    /// </summary>
    public enum GraphicsType 
    { 
        /// <summary>
        /// No extra mapping applied
        /// </summary>
        Diffuse, 

        /// <summary>
        /// Only has a Normal map
        /// </summary>
        Normal, 

        /// <summary>
        /// Only has an Emissive map
        /// </summary>
        Emissive,

        /// <summary>
        /// Has both an Emissive and Normal map
        /// </summary>
        NormalEmissive
    };

    /// <summary>
    /// A struct representing the texture coordinates (0.0 - 1.0) of an animation frame
    /// </summary>
    ...
    public class Game1 : Game
    {
        ...
        Texture2D BlankTexture, BigShellCasing, SmallShellingCasing, LightningShellCasing, Coin, RoundSparkParticle, HealthParticle,
                  ...
                  ToonBloodDrip1, ToonGlueDrip1, PaintSplat4Particle;
        ...
        List<Emitter> YSortedEmitterList, AlphaEmitterList, AdditiveEmitterList, GasEmitterList, FireEmitterList;
        ...
        protected override void Initialize()
        {
            ...
            {
                new Button(ButtonLeftSprite, new Vector2(-300, 130), null, null, null, "          play", RobotoRegular20_2, "Left", Color.White),
                new Button(ButtonLeftSprite, new Vector2(-300, 130 + ((64 + 50) * 1)), null, null, null, "          tutorial", RobotoRegular20_2, "Left", Color.White),
                new Button(ButtonLeftSprite, new Vector2(-300, 130 + ((64 + 50) * 2)), null, null, null, "          options", RobotoRegular20_2, "Left", Color.White),
                new Button(ButtonLeftSprite, new Vector2(-300, 130 + ((64 + 50) * 3)), null, null, null, "          credits", RobotoRegular20_2, "Left", Color.White),
                new Button(ButtonLeftSprite, new Vector2(-300, 1080 - 50 - 32), null, null, null, "          exit", RobotoRegular20_2, "Left", Color.White)
            };
        ...
        private void LoadGameContent()
        {
            if (GameState == GameState.Loading && IsLoading == false)
            {
                ...
                PaintSplat4Particle = Content.Load<Texture2D>("Particles/PaintSplat4");
                ...
                FireEmitterList = new List<Emitter>();
                    ...
                    case InvaderType.Soldier:
                        {
                            AnimationsList = new List<InvaderAnimation>()
                            {
                                ...
                                { 
                                    ...
                                    AnimationType = GraphicsType.Normal,
                                    Animated = true,
                                    CurrentFrame = 0,
                                    FrameDelay = 30,
                                    Looping = true,
        ...
        protected override void Draw(GameTime gameTime)
        {
                    ...
                    case GameState.Victory:
                        {
                            ...
                            //if (CurrentLevel.TrapUnlocks[0] != null)
                            {
                                string TrapName = CurrentLevel.TrapUnlocks[0].ToString() + " Trap Unlocked!";
                                spriteBatch.DrawString(RobotoRegular40_2, TrapName, new Vector2(1920 / 2, 320), Color.White, 0, RobotoBold40_2.MeasureString(TrapName) / 2, 1, SpriteEffects.None, 0);
                            }

                            //if (CurrentLevel.TrapUnlocks[0].
                            {
                                string TurretName = CurrentLevel.TurretUnlocks[0].ToString() + " Turret Unlocked!";
                                spriteBatch.DrawString(RobotoRegular40_2, TurretName, new Vector2(1920 / 2, 350), Color.White, 0, RobotoBold40_2.MeasureString(TurretName) / 2, 1, SpriteEffects.None, 0);
                            }
            ...
            if (GameState == GameState.Playing || GameState == GameState.Paused || GameState == GameState.Victory && IsLoading == false)
            {
                ...
                foreach (Drawable drawable in DrawableList)
                {
                    if (drawable.GetType().BaseType == typeof(HeavyProjectile) || drawable.GetType().BaseType == typeof(TimerHeavyProjectile))
                    {
                        drawable.Draw(GraphicsDevice, ProjectileBasicEffect, ShadowBlurEffect, ParticleEffect);
                    }
                ...
                spriteBatch.Begin(SpriteSortMode.Deferred, BlendBlack);
                spriteBatch.Draw(BlurMap, BlurMap.Bounds, Color.White);
                spriteBatch.End();

                //THIS IS HERE SO THAT WHEN THE OCCLUSION MAP IS DRAWN NEXT, THE BLENDBLACK STATE ISN'T MAINTAINED
                //IF BLENDBLACK STAYS AS THE BLENDSTATE, THE OCCLUSION OBJECTS DON'T GET DRAWN
                //GraphicsDevice.BlendState = BlendState.AlphaBlend;
                #endregion
                ...
                if (BoundingBoxes == true)
                {
                    foreach (Invader invader in InvaderList.Where(Invader => Invader.InvaderType == InvaderType.HealDrone))
                    {
                        spriteBatch.DrawString(DefaultFont, (invader as HealDrone).HealerBehaviour.ToString(), invader.Position, Color.White);
                    }
        ...
        protected override void Update(GameTime gameTime)
        {
                ...
                case GameState.Playing:
                    {
                            ...
                            PreviousKeyboardState.IsKeyDown(Keys.Enter))
                        {
                            ...
                            {
                                Powerup = new Powerup(6000, new PowerupEffect() { GeneralPowerup = GeneralPowerup.ExplosivePower, PowerupValue = 30 })
                            };
                        }
                        #endregion
                        ...
                        foreach (Rope rope in RopeList)
                        {
                            ...
                            //If the invader which created the rope has been destroyed, remove the start and end of the rope
                            if (rope.SourceObject as Invader != null)
                            {
                                if ((rope.SourceObject as Invader).CurrentHP <= 0)
                                {
                        ...
                        #region Handle FireEmitterList
                        FireEmitterList.ForEach(Emitter => Emitter.Update(gameTime));
                        FireEmitterList.RemoveAll(Emitter => Emitter.Active == false || (Emitter.AddMore == false && Emitter.ParticleList.Count == 0));
                        ...
                        if (TrapList.Count > 0)
                        {
                            ...
                            for (int t = 0; t < TrapList.Count; t++)
                            {
                                if (TrapList[t].CurrentHP <= 0 || TrapList[t].CurrentDetonateLimit == 0)
                                {
                                    ...
                                    LightList.RemoveAll(Light => Light.Tether == TrapList[t]);
                        ...
                        foreach (Drawable drawable in DrawableList.Where(Drawable => Drawable.GetType() != typeof(ShellCasing)))
                        {
                            foreach (Drawable drawable2 in DrawableList.Where(Drawable => Drawable.GetType() != typeof(ShellCasing)))
                            {
            ...
            #region Press Ctrl+Shift+V for instant victory
            ...
            #region Press Ctrl+R to start waves immediately
            if (CurrentKeyboardState.IsKeyDown(Keys.LeftControl) &&
            ...
            #region Press Ctrl+G to spawn a crate
            if (CurrentKeyboardState.IsKeyDown(Keys.LeftControl) &&
            ...
            #region Press F1 to show diagnostics
            if (CurrentMouseState.LeftButton == ButtonState.Released &&
            ...
            #region For testing CPU vs GPU binding
        ...
        public void OnExplosionHappened(object source, ExplosionEventArgs e)
        {
                ...
                switch (invader.InvaderType)
                {
                    ...
                    case InvaderType.Soldier:
                        Emitter BloodEmitter = new Emitter(SplodgeParticle,
                                            ...
                                            null, null, null, null, null, false, false, 1000, null, null, true);
                ...
                source.GetType().BaseType != typeof(HeavyRangedInvader))
            {
                ...
                foreach (Invader invader in InvaderList.Where(Invader => Vector2.Distance(Invader.Center, explosion.Position) < explosion.BlastRadius))
                {
                    ...
                    //The invaders cannot take damage from explosions created by other invaders                    
                    if ((source as Invader) == null)
                    {
                        if (trapDist > invDist)
                        {
                                ...
                                if (TowerButtonList.Contains(button))
                                {
                                    ...
                                    if (SelectedTurret != null)
                                    {
                                        Emitter Sparks1 = new Emitter(RoundSparkParticle, button.CurrentPosition,
                                            ...
                                            false, new Vector2(0, 1080), false, null, false, false, null, null, null, null, new Vector2(0.3f, 0.1f), 
                                    ...
                                    if (SelectedTurret != null)
                                    {
                                        ...
                                        if (Resources >= resourceCost && TurretList[Index] == null)
                                        {
                                            ...
                                            {
                                                OutlineTexture = TurretSelectBox
                                            };
                                    ...
                                    case ProfileManagementState.Loadout:
                                        {
                                            ...
                                            if (SelectTurretList.Contains(source as WeaponBox))
                                            {
                                                ...
                                                if (turretBox.Visible == true)
                                                {
                                                    ...
                                                    CurrentProfile.TurretDictionary.TryGetValue(turretBox.ContainsTurret.Value, out bool Available);
                                            ...
                                            if (SelectTrapList.Contains(source as WeaponBox))
                                            {
                                                ...
                                                if (trapBox.Visible == true)
                                                {
                                                    ...
                                                    //var Available = CurrentProfile.GetType().GetField(WeaponName).GetValue(CurrentProfile);
                                                    CurrentProfile.TrapDictionary.TryGetValue(trapBox.ContainsTrap.Value, out bool Available);
                        ...
                        case GameState.Victory:
                            {
                                if (button == VictoryContinue)
                                {
                                    if (CurrentProfile.LevelNumber == 1)
                                    {
                                        ...
                                        {
                                            BoxTexture = Content.Load<Texture2D>("WhiteBlock"),
                                            DialogueFont = Content.Load<SpriteFont>("Fonts/RobotoBold20_0_Outline"),
                                            TipFont = Content.Load<SpriteFont>("Fonts/RobotoLight20_0")
                                        };
        ...
        private void InvaderUpdate(GameTime gameTime)
        {
            for (int p = 0; p < InvaderList.Count; p++)
            {
                ...
                #region Damage the invaders if they walk into a gas cloud
                foreach (Emitter emitter in FireEmitterList)
                {
                    if (emitter.ParticleList.Any(Particle =>
                        Particle.DestinationRectangle.Intersects(BoundingBoxToRect(invader.CollisionBox)) &&
                        invader.DestinationRectangle.Bottom < Particle.DestinationRectangle.Bottom &&
                        ...
                        case InvaderType.Spider:
                            {
                                HeavyProjectile heavyProjectile = new AcidProjectile(heavyRangedInvader, AcidProjectileSprite, RoundSparkParticle,
                                            heavyRangedInvader.Center, heavyRangedInvader.GetLaunchVelocity(),
                                            -heavyRangedInvader.GetNextAngle(),
                                            0.2f, heavyRangedInvader.RangedDamage, new Vector2(invader.MaxY, invader.MaxY));
                        ...
                        case InvaderType.Archer:
                            {
                                HeavyProjectile heavyProjectile = new AcidProjectile(heavyRangedInvader, AcidProjectileSprite, RoundSparkParticle,
                                            ...
                                            heavyRangedInvader.GetLaunchVelocity(),
                                            -heavyRangedInvader.GetNextAngle(),
                                            0.2f, heavyRangedInvader.RangedDamage, new Vector2(invader.MaxY, invader.MaxY));
                                AddDrawable(heavyProjectile);
                            }
                        ...
                        case InvaderType.StationaryCannon:
                            {
                                ...
                                heavyProjectile = new CannonBall(
                                            heavyRangedInvader, 
                                            CannonBallProjectileSprite, 
                                            ToonSmoke3,
                                            ...
                                            new Vector2(invader.MaxY - invader.ZDepth/2, invader.MaxY - invader.ZDepth / 2));
                                
                                HeavyProjectileList.Add(heavyProjectile);
                                Debug.WriteLine("Heavy Projectile Depth: " + heavyProjectile.MaxY);
                                AddDrawable(heavyProjectile);
                            }
                            break;
                        #endregion
                        ...
                        case InvaderType.HarpoonCannon:
                            {
                                ...
                                Rope rope = new Rope(heavyRangedInvader.BarrelEnd, heavyProjectile, heavyRangedInvader, heavyRangedInvader.BoundingBox.Max.Y)
                                {
                                    DrawDepth = invader.DrawDepth - (_1Depth)
                                };
        ...
        private void HeavyProjectileUpdate(GameTime gameTime)
        {
            for (int i = 0; i < HeavyProjectileList.Count; i++)
            {
                ...
                if (heavyProjectile.Active == true && timedHeavyProjectile == null)
                {
                    ...
                    if (heavyProjectile.SourceObject as HeavyRangedInvader != null)
                    {
                        trap = TrapList.FirstOrDefault(Trap =>
                            Trap.Solid == true &&
                            (heavyProjectile.MaxY >= Trap.CollisionBox.Min.Y - (heavyProjectile.SourceObject as HeavyRangedInvader).ZDepth / 2 &&
                             heavyProjectile.MaxY <= Trap.CollisionBox.Max.Y + (heavyProjectile.SourceObject as HeavyRangedInvader).ZDepth / 2));
                    }
                    else
                    {
                        trap = TrapList.FirstOrDefault(Trap =>
                    ...
                    if (trap != null)
                    {
                        if (heavyProjectile.BoundingBox.Max.Y - heavyProjectile.BoundingBox.Min.Y <= 0)
                        {
                        }

                        if (heavyProjectile.BoundingBox.Max.X - heavyProjectile.BoundingBox.Min.X <= 0)
                        {
                        }

                        if (trap.BoundingBox.Intersects(heavyProjectile.BoundingBox))
                        {
                    ...
                    if (heavyProjectile.SourceObject.GetType().BaseType == typeof(HeavyRangedInvader))
                    {
                        ...
                        {
                            //The harpoons should not be able to hit the tower
                            //If they are able to hit the tower, then they can't 
                            //attach to the turret which is further back
                            case HeavyProjectileType.Harpoon:
                                break;
                ...
                if (heavyProjectile.Active == true && timedHeavyProjectile != null)
                {
                    ...
                    //This line was causing the TimerHeavyProjectiles to be updated twice per frame
        ...
        public void OnLightProjectileFired(object source, LightProjectileEventArgs e)
        {
                ...
                switch (LightProjectileType)
                {
                    ...
                    case LightProjectileType.Lightning:
                        for (int i = 0; i < 1; i++)
                        {
                            ...
                            {
                                DrawDepth = (CollisionEnd.Y / 1080f),
                                Color = Color.LimeGreen
                            };
                                    ...
                                    default:
                                        foreach (Invader invader in InvaderList)
                                        {
                                            if (Vector2.Distance(invader.Center, hitInvader.Center) < 150)
                                            {
                                                for (int i = 0; i < 2; i++)
                                                {
                                                    //Bolt = new LightningBolt(collisionEnd, invader.Center,Color.MediumPurple, 0.02f, 150f, true);
                                    ...
                                    case InvaderType.BatteringRam:
                                        {
                                            ...
                                            Emitter Sparks2 = new Emitter(RoundSparkParticle, collisionEnd,
                                                ...
                                                0.102f, 0.09f, 18f, 12, true, new Vector2(hitInvader.DestinationRectangle.Bottom + 4, hitInvader.DestinationRectangle.Bottom + 16), true, hitInvader.DrawDepth + (_1Depth), true, true,
            ...
            Action CheckCollision = () =>
            {
                ...
                if (sourceTurret == null && Tower.Shield.BoundingSphere.Intersects(CurrentProjectile.Ray) != null)
                {
                    SphereIntersections.Add(Tower.Shield);
                }
                ...
                #region Make sure TURRETS can't shoot themselves or other TURRETS, the TOWER or the SHIELD
                if (sourceTurret != null)
                {
                    Intersections.RemoveAll(Drawable => 
                        Drawable.GetType().BaseType == typeof(Turret) || 
                        Drawable.GetType() == typeof(Tower));
                }                
                #endregion
                ...
                Tower HitTower = hitObject as Tower;

                #region Projectile hit the GROUND or NOTHING
                if (hitObject == null)
                {
                ...
                else 
                {
                    ...
                    if (HitShield != null)
                    {
                        //TODO: THIS CODE ALSO NEEDS TO ACCOUNT FOR A TURRET PROJECTILE HITTING AN INVADER SHIELD
                        //OR A TRAP PROJECTILE HITTING A SHIELD
                        CreateEffect(sourcePosition, CollisionEnd, CurrentProjectile.LightProjectileType);
                    ...
                    if (HitTower != null)
                    {
                        ...
                        sourceInvader.HitObject = HitTower;
                ...
                if (turret.CurrentHealth <= 0)
                {
                    for (int i = 0; i < 10; i++)
                    {
                        Emitter MultiFire_Explosion5 = new Emitter(ExplosionParticle2, turret.Position,
                            ...
                            new Color(255, 128, 0, 6), new Color(0, 0, 0, 255), 0f, 1f, 37f, 3, true,
                        ...
                        DrawableList.Add(MultiFire_Explosion5);
                    }

                    //Done this way so that the DrawAble list isn't sorted 10 times in a row for no reason.
                    //Add all the emitters first and then sort the list
                    //SortDrawables();
        ...
        public void OnTurretShoot(object source, TurretShootEventArgs e)
        {
            ...
            switch (turret.TurretType)
            {
                ...
                case TurretType.MachineGun:
                    {
                        ...
                        AddDrawable(FlashEmitter, FlashEmitter2, newShell);//, puffEmitter);
                ...
                case TurretType.Cannon:
                    {
                        ...
                        Emitter Sparks1 = new Emitter(RoundSparkParticle, new Vector2(turret.BarrelEnd.X, turret.BarrelEnd.Y),
                                            new Vector2(
                                                        MathHelper.ToDegrees(-(float)Math.Atan2(Direction.Y, Direction.X)) - 15,
                                                        MathHelper.ToDegrees(-(float)Math.Atan2(Direction.Y, Direction.X)) + 15),
                                            new Vector2(2.35f, 11.4f), new Vector2(900f, 1200f), 1f, false, new Vector2(-2f, 2f),
                                            new Vector2(-1f, 1f), new Vector2(0.1f, 0.25f), new Color(255, 255, 128, 0),
                                            new Color(255, 128, 0, 255), -0.05f, 0.03f, 18f, 70, false, new Vector2(0f, 720f),
                        ...
                        HeavyProjectileList.Add(HeavyProjectile);
                        AddDrawable(HeavyProjectile, FlashEmitter, FlashEmitter2, Sparks1);
                ...
                case TurretType.Cluster:
                    {
                        ...
                        HeavyProjectile = new ClusterBombShell(turret, 1000, CannonBallProjectileSprite, ToonSmoke3,
                                new Vector2(turret.BarrelEnd.X, turret.BarrelEnd.Y), turret.LaunchVelocity,
                                turret.FireRotation, 0.35f, turret.Damage, turret.BlastRadius,
                                new Vector2(MathHelper.Clamp(turret.Position.Y + 32, 690, 930), 930));
                ...
                case TurretType.Boomerang:
                    {
                        HeavyProjectile = new BoomerangProjectile(turret, BoomerangProjectileSprite, SmokeParticle, new Vector2(turret.BarrelEnd.X, turret.BarrelEnd.Y), turret.LaunchVelocity, turret.Rotation, 0.4f, turret.Damage, 100,
                            new Vector2(MathHelper.Clamp(turret.Position.Y + 32, 690, 930), 930));
        ...
        /// <summary>
        /// Return resource cost to place a turret of a specific type
        /// </summary>
        /// <param name="trapType">The type of turret to be placed</param>
        /// <returns>The resource cost of the turret</returns>
                    ...
                    case TrapType.FlameThrower:
                        {
                            if (trap.TrapState == TrapAnimationState.Active)
                            {
                                ...
                                {
                                    Emitter Sparks1 = new Emitter(RoundSparkParticle, new Vector2(trap.Position.X + trap.CurrentAnimation.FrameSize.X - 10,
                                            trap.Position.Y + trap.CurrentAnimation.FrameSize.Y / 2), new Vector2(-3f, 3f),
                                            new Vector2(2.35f, 11.4f), new Vector2(900f, 1200f), 1f, false, new Vector2(-2f, 2f),
                                            new Vector2(-1f, 1f), new Vector2(0.1f, 0.25f), new Color(255, 255, 128, 0),
                                            new Color(255, 128, 0, 255), -0.032f, 0.03f, 18f, 70, false, new Vector2(0f, 720f),
                                            true, (trap.DestinationRectangle.Bottom - 1.0f) / 1080f, false, false, new Vector2(0f, 0f), new Vector2(0f, 0f), 0f, true,
                                    ...
                                    Emitter FireEmitter = new Emitter(ToonSmoke2, new Vector2(trap.Position.X + trap.CurrentAnimation.FrameSize.X - 10,
                                                                                              trap.Position.Y + trap.CurrentAnimation.FrameSize.Y / 2),
                                        new Vector2(-2f, 2f), new Vector2(6.85f, 9.9f),
                                        new Vector2(1200f, 1500f), 1f, false, new Vector2(0f, 0f), new Vector2(-2f, 2f), new Vector2(0.07f, 0.09f),
                                         new Color(255, 128, 0, 60), Color.Black,
                                        -0.035f, -1f, 51f, 4, true, new Vector2(trap.DestinationRectangle.Bottom, trap.DestinationRectangle.Bottom + 8), true, (trap.DestinationRectangle.Bottom - 1.0f) / 1080f,
                                        false, true, new Vector2(0f, 0f), new Vector2(0f, 0f), 0.283f, true,
                                        new Vector2(0.021f, 0.036f), false, false, 0f, false, false, true, null)
                                    {
                                        Emissive = true
                                    };
                                    ...
                                    YSortedEmitterList.Add(FireEmitter);
                                    FireEmitterList.Add(FireEmitter);
                                    AddDrawable(FireEmitter, Sparks1);
        ...
        /// <summary>
        /// What happens when an invader hits a trap
        /// </summary>
                    ...
                    case TrapType.PopFrag:
                        {
                            for (int i = 0; i < 10; i++)
                            {
                                float angle = (float)RandomDouble(0, Math.PI);
        ...
        /// <summary>
        /// What to do when the player places a trap on the terrain.
        /// </summary>
        /// <param name="source"></param>
        /// <param name="e"></param>
        ...
        /// <summary>
        /// Return resource cost to place a trap of a specific type
        /// </summary>
        /// <param name="trapType">The type of trap to be placed</param>
        /// <returns>The cost of the trap</returns>
        ...
        /// <summary>
        /// Return power cost to place a trap of a specific type
        /// </summary>
        /// <param name="trapType">The type of trap to be placed</param>
        /// <returns>The power cost of the trap</returns>
            ...
            if (CurrentWaveTime >= CurrentWave.TimeToNextWave)
            {
                ...
                if (CurrentInvaderTime >= CurrentWave.TimeToNextInvader)
                {
                    ...
                    if (CurrentInvaderIndex < CurrentWave.InvaderList.Count && StartWave == true)
                    {
                        ...
                        if (CurrentWave.InvaderList[CurrentInvaderIndex] is Invader nextInvader)
                        {
        ...
        /// <summary>
        /// To be executed when the player beats a level
        /// </summary>
        public void PlayerVictory()
        {
            ...
            VictoryContinue = new Button(ButtonLeftSprite, new Vector2(-300, 834), null, null, null, "continue", RobotoRegular20_2, "Left", null)
            {
                NextPosition = new Vector2(665, 834)
            };
            VictoryContinue.Initialize(OnButtonClick);

            UnlockTraps(CurrentLevel.TrapUnlocks.ToArray());
            UnlockTurrets(CurrentLevel.TurretUnlocks.ToArray());
        ...
        public void LoadLevel(int number)
        {
            ...
            {
                DrawDepth = 1.0f,
                BoundingBox = new BoundingBox(new Vector3(0, MathHelper.Clamp(CursorPosition.Y, 690, 1080), 0), new Vector3(1920, MathHelper.Clamp(CursorPosition.Y + 1, 800, 1080), 0))
            };
        ...
        /// <summary>
        /// Dynamically add a weapon during gameplay for the player to use. e.g. Unlock weapons during tutorial level
        /// </summary>
        /// <param name="slot">Button slot the weapon is to be inserted into</param>
        /// <param name="turret">Turret type to be inserted</param>
        /// <param name="trap">Trap type to be inserted</param>
        ...
        /// <summary>
        /// Get the corresponding icon for the powerups to display at the top of the screen
        /// </summary>
        ...
        /// <summary>
        /// Change the icon of the placed weapon button when a weapon is placed into a slot on the profile management screen
        /// </summary>
        ...
        {
            //Moved this code into SetUpWeaponSelection()
        }

        /// <summary>
        /// Set up the turret and trap select boxes for the profile management screen
        /// </summary>
        private void SetUpWeaponSelection()
        {
            ...
            for (int i = 0; i < turretTypes.Length; i++)
            {
                ...
                {
                    ContainsTurret = (TurretType)turretTypes.GetValue(i),

                    CurrencyIcon = CurrencyIcon,
                    PowerUnitIcon = PowerUnitIcon,
                    RobotoBold40_2 = RobotoBold40_2,
                    RobotoRegular20_0 = RobotoRegular20_0,
                    RobotoRegular20_2 = RobotoRegular20_2,
                    RobotoItalic20_0 = RobotoItalic20_0,
            ...
            for (int i = 0; i < trapTypes.Length; i++)
            {
                ...
                {
                    ContainsTrap = (TrapType)trapTypes.GetValue(i),
                    PowerUnitIcon = PowerUnitIcon,
                    CurrencyIcon = CurrencyIcon,
                    RobotoBold40_2 = RobotoBold40_2,
                    RobotoRegular20_0 = RobotoRegular20_0,
                ...
                switch (turretBox.ContainsTurret)
                {
                    ...
                    default:
                        ...
                        if (Available == true)
                        {
                            ...
                            turretBox.WeaponName = WeaponName;
                        }
                        else
                        {
        ...
        /// <summary>
        /// Unlock turrets at the end of a level
        /// </summary>
        /// <param name="turret">The turrets to be unlocked</param>
        public void UnlockTurrets(params TurretType[] turret)
        {
            for (int i = 0; i < turret.Length; i++)
            {
                if (!CurrentProfile.TurretDictionary.ContainsKey(turret[i]))
                    CurrentProfile.TurretDictionary.Add(turret[i], true);
            }
        }

        /// <summary>
        /// Unlock traps at the end of a level
        /// </summary>
        /// <param name="trap">The traps to be unlocked</param>
        public void UnlockTraps(params TrapType[] trap)
        {
            for (int i = 0; i < trap.Length; i++)
            {
                if (!CurrentProfile.TrapDictionary.ContainsKey(trap[i]))
                    CurrentProfile.TrapDictionary.Add(trap[i], true);
            }
        }
        ...
        public void AddNewProfile()
        {
            ...
            foreach (TurretType turretType in Enum.GetValues(typeof(TurretType)))
            {
                CurrentProfile.TurretDictionary.TryGetValue(turretType, out bool myThing);
            }

            foreach (TrapType trapType in Enum.GetValues(typeof(TurretType)))
            {
                CurrentProfile.TrapDictionary.TryGetValue(trapType, out bool myThing);
            }
